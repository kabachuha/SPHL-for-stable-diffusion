...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="l2",
        choices=["l2", "huber", "huber_scheduled", "huber_scheduled_backwards", "pure_huber", "pure_huber_scheduled", "pure_huber_scheduled_backwards"],
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if 'scheduled' in args.loss_type:
                    timesteps = torch.randint(
                        0, noise_scheduler.config.num_train_timesteps, (1,), device='cpu'
                    )
                    timestep = timesteps.item()

                    if 'backwards' in args.loss_type:
                        timestep = noise_scheduler.config.num_train_timesteps - timestep

                    alpha = - math.log(args.huber_c) / noise_scheduler.config.num_train_timesteps
                    huber_c = math.exp(-alpha * timestep)
                    timesteps = timesteps.repeat(bsz).to(model_input.device)
                else:
                    # HACK: for fairness in comparison 
                    timesteps = torch.randint(
                        0, noise_scheduler.config.num_train_timesteps, (1,), device='cpu'
                    )
                    timesteps = timesteps.repeat(bsz).to(model_input.device)
                ...
                if args.with_prior_preservation:
                    ...
                    if args.loss_type == "l2":
                        loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
                    elif "huber" in args.loss_type and not "pure" in args.loss_type:
                        loss = torch.mean(
                            huber_c * (torch.sqrt((model_pred.float() - target.float()) ** 2 + huber_c**2) - huber_c)
                        )
                    elif "pure_huber" in args.loss_type:
                        loss = F.huber_loss(model_pred.float(), target.float(), reduction="mean", delta=huber_c)
                    else:
                        raise ValueError(f'unknown loss {args.loss_type}')
